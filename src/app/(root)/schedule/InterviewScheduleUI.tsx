...
import { FileSearch, Loader2Icon, XIcon } from "lucide-react";
...
const InterviewScheduleUI = () => {
    ...
    const addInterviewer = (interviewerId: string) => {
        if (!formData.interviewerIds.includes(interviewerId)) {
            setFormData((prevFormData) => ({
                ...prevFormData,
                interviewerIds: [...prevFormData.interviewerIds, interviewerId],
            }));
        }
    }
    ...
    const removeInterviewer = (interviewerId: string) => {
        setFormData((prevFormData) => ({
            ...prevFormData,
            interviewerIds: prevFormData.interviewerIds.filter((id) => id !== interviewerId),
        }));
    }

    const selectedInterviewers = interviewers?.filter((interviewer) => formData.interviewerIds.includes(interviewer.clerkId));

    const availableInterviewers = interviewers?.filter((interviewer) => !formData.interviewerIds.includes(interviewer.clerkId));
return (
    <div className="container max-w-7xl mx-auto p-6 space-y-8">
        ...
        {!interviews ? (
            ...
            <h2 className="text-lg font-semibold mt-4">No Interviews Scheduled</h2>
            <p className="text-sm text-gray-500 mt-2">
              Add interviews to keep track of your schedule.
            </p>
          </div>
        )}
